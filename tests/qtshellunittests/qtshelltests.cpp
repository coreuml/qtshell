...
QtShellTests::QtShellTests(QObject *parent) : QObject(parent) {
  auto ref = [=]() {
    QTest::qExec(this, 0,
                 0); // Autotest detect available test cases of a QObject by
                     // looking for "QTest::qExec" in source code
  };
  Q_UNUSED(ref);
}
...
void QtShellTests::test_bulk() {
  ...
  auto predicate = [&](const QString &from, const QString &to,
...
void QtShellTests::test_find() {
  ...
  /*QVERIFY(files.filter(QRegExp("*.h", Qt::CaseInsensitive, QRegExp::Wildcard))
              .size() == files.size());*/

  QVERIFY(files
              .filter(QRegularExpression::fromWildcard(
                  QStringView(QString("*.h")), Qt::CaseInsensitive,
...
void QtShellTests::test_cat() {
  ...
  content = cat(QStringList() << "cat.txt"
                              << "cat.txt");
  QVERIFY(content == "0123456789\n0123456789");
...
void QtShellTests::test_realpath_strip() {
  ...
  QCOMPARE(QtShell::realpath_strip("tmp", "subdir1"),
           (QtShell::pwd() + "/tmp/subdir1"));

  QCOMPARE(QtShell::realpath_strip("tmp", "subdir1", "subdir2"),
           (QtShell::pwd() + "/tmp/subdir1/subdir2"));
  ...
  QCOMPARE(QtShell::realpath_strip("file:///E:/dir/tmp.txt"),
           QString("E:/dir/tmp.txt"));
  QCOMPARE(QtShell::realpath_strip("file://networkdrive/tmp1.txt"),
           QString("//networkdrive/tmp1.txt"));
  QCOMPARE(QtShell::realpath_strip("\\\\networkdrive\\tmp1.txt"),
           QString("//networkdrive/tmp1.txt"));
  QCOMPARE(QtShell::realpath_strip("//networkdrive/tmp1.txt"),
           QString("//networkdrive/tmp1.txt"));

  // QFileDialog::getOpenFileName returns folder in a way like
  // //networkdrive/tmp1.txt
#else
  /// The no. of "/" is critical for windows system. It help to determine is it
  /// a network drive
...
#ifdef Q_OS_WIN32
  QCOMPARE(QtShell::which("ping").toLower(),
           QString("c:\\Windows\\System32\\PING.EXE").toLower());
#endif
}
